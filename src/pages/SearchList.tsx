...
export default function SearchList() {
  ...
  const getData = async () => {
    ...
    const text =
      inputRef.current && (inputRef.current.value !== '' ? inputRef.current.value : BLANK);
    ...
    if (resultCode === 'INFO-000' || resultCode === 'INFO-200') {
      ...
      const totalCount = data.culturalEventInfo?.list_total_count;
  ...
  const dateSelectFn = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    <main className="bx_main">
      ...
      <div className="bx_srch_result">
        <div className="bx_search_form">
          <span>
            <label className="text_label" htmlFor="dateSelect">
              날짜
            </label>
            <select
              onChange={dateSelectFn}
              id="dateSelect"
              defaultValue={getDate ? 'select' : 'all'}
            >
            ...
            <>
              ...
              {data ? (
                data.map((item: any, index: number) => {
                  return (
                    <ArticleListItem key={index} {...item} handleClick={() => listClick(item)} />
                  );
                })
              ) : (
