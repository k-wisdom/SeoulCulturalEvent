...
export function usePagination(dataCnt: number, LISTCOUNT = 10, LIMIT = 10, startNum = 0) {
  const [pageNum, setPageNum] = useState<number>(startNum);
  ...
  const PageBtnClickFn = (event: React.MouseEvent) => {
  ...
  const paging = () => {
    ...
    btnArr.push(
      <button
        key={'prev'}
        className="btn_prev"
        disabled={pageRef.current === 1 && true}
        onClick={() => pageArrowClickFn('prev')}
      >
    ...
    for (let i = (page - 1) * LIMIT; i < LIMIT * page && i < totalNumCount; i++) {
      ...
      if (i === pageNum) {
        //해당 페이지에 active
        btnArr.push(
          <button key={key} className="active" onClick={PageBtnClickFn}>
            {i + 1}
          </button>,
        );
      } else {
        btnArr.push(
          <button key={key} onClick={PageBtnClickFn}>
            {i + 1}
          </button>,
        );
      }
    ...
    btnArr.push(
      <button
        key={'next'}
        className="btn_next"
        disabled={pageRef.current === pageNumCount && true}
        onClick={() => pageArrowClickFn('next')}
      >
  ...
  const Pagination = (
    <div className="paging_wrap">
      <div className="number_wrap">{paging()}</div>
    </div>
  );
